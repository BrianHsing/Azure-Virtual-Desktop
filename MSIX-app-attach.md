...
## WVD 使用 MSIX 應用程式連接的優點
  MSIX 應用程式會寫入至自己的虛擬登錄和應用程式資料檔案夾並且隔離，並在解除安裝或重設應用程式時刪除此資料，其他應用程式無法存取 MSIX 應用程式的虛擬登錄或虛擬檔案系統。MSIX 應用程式可讓企業保持最新狀態，並確保其應用程式永遠保持最新狀態。其可讓 IT 專業人員和開發人員提供以使用者為中心的解決方案，同時藉由減少重新封裝的需求，來降低應用程式的擁有成本。<br>
## 設定 MSIX 應用程式連接的先決條件
 - 了解什麼是 MSIX？<br>
